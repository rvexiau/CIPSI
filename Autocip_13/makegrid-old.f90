...
subroutine makegrid(ncore,M,molecaxis,refdist,group,grid_method,coordinate,lower_bound,grid_file)
   ...
   real(dp) :: M(ncore),Masseff,Mtot,d1,d2,d3,dist(ncoord),angle1,angle2,pi,refdist(ncoord),lower_bound,temp,temp2,kmat(3,3),xmat(6),massfact(9)
...
! Automatic placement of molecular axis
  if(group=='CS'.or.group=='cs'.or.group=='CNV'.or.group=='cnv'.or.group=='CNH'.or.group=='cnh'.or.group=='DNH'.or.group=='dnh')then 
   ...
   do k = 1, npoint      
     ...
     ! convert to cartesian, see Ragni2007 (corrected typo in Ragni : (mb+mb) and (mc+mc) replaced by mb+mc)
!       coord_cart(:,3,i) = 0d0
!       Masseff=M(3)+M(2)
!       temp=(4.*M(2)*Masseff*(dist(1)/M(3))**2+4.*Masseff*dist(2)**2/M(3)-4*M(2)*dist(3)**2/M(3))
!             write(*,*) dist(1),dist(2),dist(3)
!      write(*,*) M(1),M(2),M(3)
!       write(*,*) Masseff
!       write(*,*) temp
!       if(temp.eq.0d0) then
...
!       else
!         coord_cart(1,1,i) = - dsqrt(abs(dist(1)**2*M(2)/Masseff+dist(2)**2*M(3)/Masseff-dist(3)**2*M(2)*M(3)/((M(2)+M(3))**2)))
!         coord_cart(1,2,i) = 0d0     
!         coord_cart(2,1,i) = (dist(1)**2-dist(2)**2+dist(3)**2*(M(2)-M(3))/Masseff)/dsqrt(abs(temp))
!         coord_cart(2,2,i) = -dsqrt(abs((-dist(2)**4-dist(3)**4-dist(1)**4+2.*(dist(2)*dist(1))**2+2.*(dist(2)*dist(3))**2+2.*(dist(3)*dist(1))**2 )) / abs(temp))
!         coord_cart(3,1,i) = (dist(2)**2-dist(1)**2+dist(3)**2*(M(2)-M(3))/Masseff)/dsqrt(abs(temp)*(M(3)/M(2))**2)
!         coord_cart(3,2,i) = dsqrt(abs((-dist(2)**4-dist(3)**4-dist(1)**4+2.*(dist(2)*dist(1))**2+2.*(dist(2)*dist(3))**2+2.*(dist(3)*dist(1))**2 )) / (abs(temp)*(M(3)/M(2))**2))
         
         ! In Ragni2007, center = center of mass of BC
         ! translate system so that center = center of mass ABC
!         temp= -M(1)*coord_cart(1,1,i)/(M(2)+M(3)+M(1))
!         coord_cart(1,1,i)=coord_cart(1,1,i)+temp
!         coord_cart(2,1,i)=coord_cart(2,1,i)+temp
!         coord_cart(3,1,i)=coord_cart(3,1,i)+temp           
!       endif
!       write(*,*) (dist(2)**2-dist(1)**2+dist(3)**2*(M(2)-M(3))/Masseff)
!       write(*,*) (abs(temp)*(M(3)/M(2))**2)
!       write(*,*)  coord_cart(3,1,i),coord_cart(3,2,i)
     ...
     case (4)
       if((coordinate(1:1).eq.'j').or.(coordinate(1:1).eq.'J')) then  
        ...
        ! vector r12 and r34 defined by spherical coord. (choice : phi1=0)
         ...
         dist(2) = sqrt(abs(4*temp_coord(1,k)**2 + temp_coord(2,k)**2 + temp_coord(3,k)**2 +4*temp_coord(1,k)*temp_coord(2,k)*Cos(theta1)-4*temp_coord(1,k)*temp_coord(3,k)*Cos(theta2)-2*temp_coord(2,k)*temp_coord(3,k)*Cos(theta1)*Cos(theta2)-2*temp_coord(2,k)*temp_coord(3,k)*Sin(theta1)*Sin(theta2)*Cos(phi2)))/2d0
         dist(3) = sqrt(abs(4*temp_coord(1,k)**2 + temp_coord(2,k)**2 + temp_coord(3,k)**2 +4*temp_coord(1,k)*temp_coord(2,k)*Cos(theta1)+4*temp_coord(1,k)*temp_coord(3,k)*Cos(theta2)+2*temp_coord(2,k)*temp_coord(3,k)*Cos(theta1)*Cos(theta2)+2*temp_coord(2,k)*temp_coord(3,k)*Sin(theta1)*Sin(theta2)*Cos(phi2)))/2d0
         dist(4) = sqrt(abs(4*temp_coord(1,k)**2 + temp_coord(2,k)**2 + temp_coord(3,k)**2 -4*temp_coord(1,k)*temp_coord(2,k)*Cos(theta1)-4*temp_coord(1,k)*temp_coord(3,k)*Cos(theta2)+2*temp_coord(2,k)*temp_coord(3,k)*Cos(theta1)*Cos(theta2)+2*temp_coord(2,k)*temp_coord(3,k)*Sin(theta1)*Sin(theta2)*Cos(phi2)))/2d0
         dist(5) = sqrt(abs(4*temp_coord(1,k)**2 + temp_coord(2,k)**2 + temp_coord(3,k)**2 -4*temp_coord(1,k)*temp_coord(2,k)*Cos(theta1)+4*temp_coord(1,k)*temp_coord(3,k)*Cos(theta2)-2*temp_coord(2,k)*temp_coord(3,k)*Cos(theta1)*Cos(theta2)-2*temp_coord(2,k)*temp_coord(3,k)*Sin(theta1)*Sin(theta2)*Cos(phi2)))/2d0
         dist(6) = temp_coord(3,k)
       elseif((coordinate(1:1).eq.'k').or.(coordinate(1:1).eq.'K')) then  
        ...
        ! Jacobi : R, r13, r24 , theta1,theta2,phi2
         ...
         dist(1) = sqrt(abs(4*temp_coord(1,k)**2 + temp_coord(2,k)**2 + temp_coord(3,k)**2 +4*temp_coord(1,k)*temp_coord(2,k)*Cos(theta1)-4*temp_coord(1,k)*temp_coord(3,k)*Cos(theta2)-2*temp_coord(2,k)*temp_coord(3,k)*Cos(theta1)*Cos(theta2)-2*temp_coord(2,k)*temp_coord(3,k)*Sin(theta1)*Sin(theta2)*Cos(phi2)))/2d0
         dist(3) = sqrt(abs(4*temp_coord(1,k)**2 + temp_coord(2,k)**2 + temp_coord(3,k)**2 +4*temp_coord(1,k)*temp_coord(2,k)*Cos(theta1)+4*temp_coord(1,k)*temp_coord(3,k)*Cos(theta2)+2*temp_coord(2,k)*temp_coord(3,k)*Cos(theta1)*Cos(theta2)+2*temp_coord(2,k)*temp_coord(3,k)*Sin(theta1)*Sin(theta2)*Cos(phi2)))/2d0
         dist(4) = sqrt(abs(4*temp_coord(1,k)**2 + temp_coord(2,k)**2 + temp_coord(3,k)**2 -4*temp_coord(1,k)*temp_coord(2,k)*Cos(theta1)-4*temp_coord(1,k)*temp_coord(3,k)*Cos(theta2)+2*temp_coord(2,k)*temp_coord(3,k)*Cos(theta1)*Cos(theta2)+2*temp_coord(2,k)*temp_coord(3,k)*Sin(theta1)*Sin(theta2)*Cos(phi2)))/2d0
         dist(6) = sqrt(abs(4*temp_coord(1,k)**2 + temp_coord(2,k)**2 + temp_coord(3,k)**2 -4*temp_coord(1,k)*temp_coord(2,k)*Cos(theta1)+4*temp_coord(1,k)*temp_coord(3,k)*Cos(theta2)-2*temp_coord(2,k)*temp_coord(3,k)*Cos(theta1)*Cos(theta2)-2*temp_coord(2,k)*temp_coord(3,k)*Sin(theta1)*Sin(theta2)*Cos(phi2)))/2d0
