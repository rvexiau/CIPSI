      subroutine tpqkls(mp,mq,mr,ms,mk,ml,ypq,yrs,ypqrs,ykl,yijkl,kc,lc
     y,bijkls)
...
21    rs=0
      ...
      if(ypqrs) go to 30
      mij=mpqm*(nrs-1)
      tab(sr+s)=bijkls(mij+npq)
c
      go to 100
30    if(npq.lt.nrs) go to 40
      ijkl=numnu(npq)+nrs
      tab(sr+s)=bijkls(ijkl)
      go to 100
40    tab(sr+s)=bijkls(numnu(nrs)+npq)
      go to 100
60    nrs=nrs+1
      if(ypqrs) go to 70
      pqrs=bijkls(mij+npq)
