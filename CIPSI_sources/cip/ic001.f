...
c--- lecture sur f59 des vp et valp issues de bdav, puis reclassement   
      ...
      read(4) (iord60(k),k=1,mcf)    
       ...
       do 5392 k=1,ncf                                                  
       kk=iord60(k)                                                     
5392   hdiag(kk)=c(mncf+k)                                              
       do 5394 k=1,ncf                                                  
5394   c(mncf+k)=hdiag(k)                                               
5390   continue                                                         
       do 5594 i=1,ncf                                                  
5594   hdiag(i)=hdig(i)                                                 
       else                                                             
...
c                                                                       
      call degen(letat)                                                 
c                                                                       
c  selection etats de spin                                              
c                                                                       
      do 1364 k=1,metat                                                 
 1364 iord60(k)=numero(k)                                               
      if (mspin(1).ne.0) then                                           
      write(6,*) 'selection des etats de spin '                         
      write(6,*) 'multiplicite'                                         
      write(6,1357) (mspin(kk),kk=1,metat)                              
1357  format(10(1x,i3))                                                 
      call spasym(letat)                                                
      call mult(fmul,letat)                                             
      ksel=0                                                            
      mslast=0                                                          
      do 2359 i=1,letat                                                 
2359  iord60(i)=i                                                       
      do 1356 kk=1,metat                                                
      if(mspin(kk).eq.0) go to 1358                                     
      if(mspin(kk).ne.mslast) iideb=1                                   
      do 1354 ii=iideb,letat                                            
         iis=int(0.5+2.*cnvmul(fmul(ii)))+1                             
         if (iis.eq.mspin(kk)) then                                     
            iord60(kk)=ii                                               
            numero(kk)=ii                                               
            ksel=ksel+1                                                 
           go to 1359                                                   
...
1356   continue                                                         
1358  continue                      
      if(ksel.ne.metat) then                                            
      write(6,*) ' erreur sur  la multiplicite de spin (mspin(i)'       
      write(6,*) ' selectionnes ',ksel                                  
      stop                                                              
      endif                                                             
      write(6,*) 'nombre d etats selectionnes: ',metat                  
      write(6,*) 'numeros des etats selectionnes '                      
      write(6,8230) (numero(jj),jj=1,metat)                             
8230  format(10(3x,i2))                                                 
      endif                                                             
c                                                                       
c  reclassement suivant numero                                          
c                                                                       
      do 750 i=1,metat                                                  
      j=iord60(i)                                                       
      eei=e(i)                                                          
      ...
      c(nncf+k)=cc                                                      
      cc=dabs(cc)                                                       
      if(cc.ge.coeff) ygen(k)=.true.                                    
751   continue                                                          
      do 753 k=i+1,letat                                                
      if(iord60(k).eq.i) iord60(k)=j                                    
753   continue                                                          
750   continue                                                          
      nbr=metat*(metat+1)/2                                             
      write(6,5888)                                                     
...
6822  continue                                                          
      ...
      mono1=1+nd(k)                                                     
      mono2=mono1+ne(k)-1                                               
      write(6,1400) mig,(c(ncf*(j-1)+k),j=max1,max)                     
      write(6,6666) (iorb(trou(i)),iwspin(trou(i)),i=mono1,mono2),      
     * ibla1,ibla2, (iorb(part(i)),iwspin(part(i)),i=mono1,mono2)       
112   continue                                                          
