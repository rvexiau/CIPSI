...
C     real*8 function xyzint(lx,ly,lz,rtab)
      real*8 function xyzint(lx,ly,lz)
      implicit none
...
C This table contains 512/Pi*Int_0^2Pi dphi Sin^k(phi) Cos^l(phi)
     ...
     &     0,   0,   0,   0,   0,   0,   0,  0/
      ...
      if      (lz.eq.0) then
C k = lx+ly+1; l= lx+ly
        xyzint = ph*rtab((lx+ly+1)*(lx+ly+2)/2,1)

      else if (lz.eq.1) then
C k = lx+ly+1; l= lx+ly+1
        xyzint = ph*rtab((lx+ly+1)*(lx+ly+4)/2,2)

      else if (lz.eq.2) then
C k = lx+ly+1; l= lx+ly+2 and k=lx+ly+3; l=lx+ly+2
        xyzint = ph*(rtab((lx+ly+2)*(lx+ly+5)/2 - 1,1) - 
     &               rtab((lx+ly+2)*(lx+ly+5)/2 + 1,1))

      else if (lz.eq.3) then
C k = lx+ly+1; l= lx+ly+3 and k=lx+ly+3; l=lx+ly+3
        xyzint = ph*(rtab((lx+ly+3)*(lx+ly+6)/2 - 2,2) - 
     &               rtab((lx+ly+3)*(lx+ly+6)/2    ,2))

      else if (lz.eq.4) then
C k = lx+ly+1; l= lx+ly+4, k=lx+ly+3; l=lx+ly+4 and k=lx+ly+5; l=lx+ly+4
        xyzint = ph*(rtab((lx+ly+3)*(lx+ly+8)/2 - 1,1) - 
     &            2.*rtab((lx+ly+3)*(lx+ly+8)/2 + 1,1) + 
     &               rtab((lx+ly+3)*(lx+ly+8)/2 + 3,1))

      else if (lz.eq.5) then
C k = lx+ly+1; l= lx+ly+5, k=lx+ly+3; l=lx+ly+5 and k=lx+ly+5; l=lx+ly+5
        xyzint = ph*(rtab((lx+ly+4)*(lx+ly+9)/2 - 2,2) - 
     &            2.*rtab((lx+ly+4)*(lx+ly+9)/2    ,2) + 
     &               rtab((lx+ly+4)*(lx+ly+9)/2 + 2,2))

      else if (lz.eq.6) then
C k = lx+ly+1; l= lx+ly+6, k=lx+ly+3, k=lx+ly+5 and k=lx+ly+7
        xyzint = ph*(rtab((lx+ly+4)*(lx+ly+11)/2    ,1) - 
     &            3.*rtab((lx+ly+4)*(lx+ly+11)/2 + 2,1) + 
     &            3.*rtab((lx+ly+4)*(lx+ly+11)/2 + 4,1) - 
     &               rtab((lx+ly+4)*(lx+ly+11)/2 + 6,1))

      else if (lz.eq.7) then
C k = lx+ly+1; l= lx+ly+7, k=lx+ly+3, k=lx+ly+5 and k=lx+ly+7
        xyzint = ph*(rtab((lx+ly+5)*(lx+ly+12)/2 - 1,2) - 
     &            3.*rtab((lx+ly+5)*(lx+ly+12)/2 + 1,2) + 
     &            3.*rtab((lx+ly+5)*(lx+ly+12)/2 + 3,2) - 
     &               rtab((lx+ly+5)*(lx+ly+12)/2 + 5,2))
      end if
      ...
      do lx=0,7
        do ly=0,7
          do lz=0,7
            if ((lx+ly+lz.ge.1).and.(lx+ly+lz.le.7)) then
              x = xyzint(lx,ly,lz)
            end if
          end do
        end do
...
C    only for test
C
      real*8 function rtab(i,ic) 
      integer i,ic
      integer k,l

      l = int((sqrt(8*i-7.)-1.)/2+.001)
      k = i-l*(l+1)/2
