...
cRG      integer*2 nttt
      integer*4 nttt
      integer*4 ne,nd,ndcip,iorb,ispin,itsym,its                 
      integer*4 trou,part
      ...
      character*40 typdip
! ###
! DB modif pour imprimer les energies de 'bdvec...' en clair
!      real*8 energ( metz2 )
!      data energ / metz2*0.d0 /
! ###
      common/uni/yuni
...
c      data charnum/'1','2','3','4','5','6','7','8','9'/
     ...
     & ietat,jetat,titre,ydipol,yat,yona,calfa,
...
33    jetat(i)=0                                                        
      ...
      if(ydipol) yat=.true.                                             
      if(yuni)then                                                      
      write(6,*) 'fichiers uniques'
      n2fic=n1fic                                                       
      n2fim=n1fim                                                       
      n2fid=n1fid
      n2fids=n1fids
      else
      write(6,*) 'fichiers differents'
      endif                                                             
...
950   format(///3x,90('*')/3x,'*',88x,'*'/3x,'    calcul de la ',       
     ...
     * 3x,'*',4x,a20,4x,'*'/3x,'*',88x,'*'/3x,90('*')///)              
     ...
     &,(ne(j),ndcip(j),j=1,ncf1),(trou(j),part(j),j=1,ii)
      do i=1,ncf1
	 nd(i)=ndcip(i)
       enddo
       write(6,*) ' cipsi '                                             
...
! ##############
! DB : Impression des energies venant du 1er fichier 'bdvec...' (unite 58) en clair 
!       energ( 1 : metat1 ) = e( 1 : metat1 )
!       write( 6, '(f13.8)' ) energ( 1 : metz2 )       
!       open( file = 'Energies_ciro', unit = 258, status = 'unknown',
!     &action = 'write', position = 'append' )
!       write( 258, * ) 'Energies du fichier "a" :'
       !write( 258, '(<metat1>(2X, f11.8, /))' ) energ( 1 : metat1 )
       !write( 258, '(<metz2>(2X, f11.8, /))' ) energ( 1 : metz2 )
!       write( 258, * ) energ( 1 : metz2 )
!       close( 258 )
! ##############
! RV 2016 : read file 64 of moy instead of 61    
      read(n1fids) ncf1,ii,mnorb,yion,(ne(i),nd(i),i=1,ncf1),
...
! ##############
! DB : Impression des energies venant du 2e fichier 'bdvec...' (unite 61) en clair 
!       energ( metz + 1 : metz + metat2 ) =
!     &e( metz + 1 : metz + metat2 )
!       write( 6, '(f13.8)' ) energ( 1 : metz2 )
!       open( file = 'Energies_ciro', unit = 258, status = 'unknown',
!     &action = 'write', position = 'append' )
!       write( 258, * ) 'Energies du fichier "b" :'
!       write( 258, '(<metat2>(2X, f11.8, /))' )
!     &energ( metz + 1 : metz + metat2 )
!       write( 258, '(<metz2>(2X, f11.8, /))' ) energ( 1 : metz2 )
!       write( 258, * ) energ( 1 : metz2 )       
!       close( 258 )
! ##############     
! RV 2016 : read file 64 of moy instead of 61
       read(n2fids) ncf2,iiii,mnorb,yion,
     *(ne(i+ncf1),nd(i+ncf1),i=1,ncf2),
     *(trou(i+ii),part(i+ii),i=1,iiii)  
...
cwr       write(6,7876) (cc,i=1,ncf2)                                   
      ...
      write(6,*) 'fin reijkl'
      ndd=nao*nao     
! RV  Bug fixed, ii had no values in older version    
      do 440 k=ncf1+1,ncf                                               
440   nd(k)=nd(k)+ii                                                    
      ncou=0                                                            
      do 25 i=1,ncouz                                                   
...
c                                                                       
      ...
      ij=(j-1)*norb + i                                                 
 881  ccc(kon)=-r(ij)        
!     RV 2016 : replace given by Lapack 
      call diagonaliser(norb,ccc,ccc(nao*nao+1),ccc(nao*nao+nao+1),norb)       
...
c                                                                       
 ...
 9998 format(/,80(1h*),/,8(10h fin ciro ),/, 10x,a25,/,
     * 80(1h*))
      stop                                                              
